...
import { getTerrainCrpGain, getTerrainSyncChange, getTerrainStaRecovery } from '../../core/metaStats';
          ...
          >
            <div className="debug-screen__entry-info">
              ...
              <span className="debug-screen__entry-meta">
                <span style={{ color: getCostColor(data.movementCost), fontWeight: 700, fontSize: 12 }}>
...
function TerrainDetail({ terrainKey, data }: { terrainKey: TerrainType; data: typeof TERRAIN[TerrainType] }) {
    ...
    <div data-testid={`debug-detail-${terrainKey}`}>
      ...
      <div className="debug-screen__section">
        ...
        <table className="debug-screen__table">
          ...
          <tbody>
            <tr><td>Normal</td><td style={{ color: getCostColor(baseCost) }}>{baseCost >= 99 ? 'Impassable' : baseCost}</td></tr>
            <tr><td>Mounted</td><td style={{ color: getCostColor(mountedCost) }}>{mountedCost >= 99 ? 'Impassable' : mountedCost}</td></tr>
            <tr><td>Flying</td><td style={{ color: getCostColor(flyingCost) }}>{flyingCost >= 99 ? 'Impassable' : flyingCost}</td></tr>
            <tr><td>Armored</td><td style={{ color: getCostColor(armoredCost) }}>{armoredCost >= 99 ? 'Impassable' : armoredCost}</td></tr>
          </tbody>
        </table>
      </div>
        ...
        <div className="debug-screen__stats-grid">
          <div className="debug-screen__stat-row">
            ...
            <div className="debug-screen__stat-bar-track">
              <div className="debug-screen__stat-bar-fill debug-screen__stat-bar-fill--blue" style={{ width: `${Math.min(100, (data.defenseBonus / 5) * 100)}%` }} />
            </div>
          </div>
          <div className="debug-screen__stat-row">
            ...
            <div className="debug-screen__stat-bar-track">
              <div className="debug-screen__stat-bar-fill debug-screen__stat-bar-fill--gold" style={{ width: `${Math.min(100, Math.max(0, (data.avoidBonus / 30) * 100))}%` }} />
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="debug-screen__section">
        ...
        <p className="debug-screen__desc-text">
          Base: <span style={{ color: basePassable ? '#22c55e' : '#ef4444', fontWeight: 700 }}>{basePassable ? 'Passable' : 'Impassable'}</span>
        </p>
          ...
          <div className="debug-screen__stats-grid">
            {crp !== 0 && (
              <div className="debug-screen__stat-row">
                ...
                <span className="debug-screen__stat-value" style={{ color: '#ef4444' }}>{signed(crp)}</span>
              </div>
            )}
            {sync !== 0 && (
              <div className="debug-screen__stat-row">
                <span className="debug-screen__stat-label">SYNC</span>
                <span className="debug-screen__stat-value" style={{ color: sync > 0 ? '#22c55e' : '#ef4444' }}>{signed(sync)}</span>
              </div>
            )}
            {sta !== 0 && (
              <div className="debug-screen__stat-row">
                <span className="debug-screen__stat-label">STA</span>
                <span className="debug-screen__stat-value" style={{ color: '#22c55e' }}>{signed(sta)}</span>
              </div>
            )}
          </div>
        ) : (
