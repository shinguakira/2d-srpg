...
- [x] In `UnitStatsPanel.tsx`: after DEF/AVO lines, compute movement cost
- [x] If a unit is selected (`selectedUnitId`), look up class flags from `ALL_CLASSES[unit.classId]` and call `getClassMovementCost(terrain, classFlags)`
- [x] If no unit selected, use base cost from `getTerrainData(terrain).movementCost`
- [x] Display "Move: {cost}" line — or "Move: ---" in red if cost >= 99 (impassable)
- [x] Show class type label in parentheses when unit-specific: "(Mounted)", "(Flying)", "(Armored)", "(Infantry)"
- [x] Import `getClassMovementCost` from `src/core/terrain.ts` (already exported)
- [x] Import `ALL_CLASSES` from `src/data/promotedClasses.ts` for class flag lookup
- [x] `data-testid="terrain-move-cost"` on the movement cost element
- [x] Test: select cavalier → hover forest → "Move: 3 (Mounted)"
- [x] Test: select pegasus → hover mountain → "Move: 1 (Flying)"
- [x] Test: select knight → hover mountain → "Move: --- (Armored)" in red
- [x] Test: no unit selected → hover forest → "Move: 2"

## Spec Update

- [x] Confirm `specs/ui/hud.md` Terrain Info section mentions movement cost
