import { CH1_SCRIPTS } from '../../story/chapters/ch1';
import type { ChapterDef } from '../chapters';
import type { Seed } from '../roster';
import type { Stats } from '../../types';

function st(
  hp: number,
  str: number,
  mag: number,
  skl: number,
  spd: number,
  lck: number,
  def: number,
  res: number,
  con: number,
  mov: number,
): Stats {
  return { hp, str, mag, skl, spd, lck, def, res, con, mov };
}

const ENEMIES: Seed[] = [
  {
    id: 'e_b1',
    name: '山賊',
    classId: 'brigand',
    level: 3,
    x: 12,
    y: 9,
    affinity: 'fire',
    stats: st(24, 8, 0, 4, 5, 1, 3, 0, 13, 5),
    growth: st(70, 45, 0, 30, 30, 15, 20, 5, 0, 0),
    weapons: ['ironAxe'],
    ai: 'aggressive',
  },
  {
    id: 'e_b2',
    name: '山賊',
    classId: 'brigand',
    level: 2,
    x: 13,
    y: 5,
    affinity: 'fire',
    stats: st(22, 7, 0, 4, 5, 1, 3, 0, 13, 5),
    growth: st(70, 45, 0, 30, 30, 15, 20, 5, 0, 0),
    weapons: ['handAxe'],
    ai: 'aggressive',
  },
  {
    id: 'p_corwin',
    name: 'コルウィン',
    classId: 'mercenary',
    level: 4,
    x: 5,
    y: 6,
    affinity: 'dark',
    stats: st(23, 7, 0, 9, 9, 3, 5, 1, 9, 5),
    growth: st(70, 45, 0, 55, 55, 25, 25, 15, 0, 0),
    weapons: ['ironSword'],
    wexp: { sword: 90 },
    ai: 'guard',
    recruitableBy: 'p_shigeru',
  },
  {
    id: 'e_a1',
    name: '狩人',
    classId: 'eArcher',
    level: 3,
    x: 15,
    y: 8,
    affinity: 'wind',
    stats: st(19, 6, 0, 8, 7, 2, 3, 1, 7, 5),
    growth: st(60, 40, 0, 55, 45, 25, 20, 15, 0, 0),
    weapons: ['ironBow'],
    ai: 'guard',
  },
  {
    id: 'e_s1',
    name: '暗黒魔道士',
    classId: 'shaman',
    level: 3,
    x: 7,
    y: 5,
    affinity: 'dark',
    stats: st(19, 1, 7, 6, 6, 2, 3, 6, 6, 5),
    growth: st(55, 5, 55, 45, 40, 20, 15, 45, 0, 0),
    weapons: ['flux'],
    ai: 'guard',
  },
  {
    id: 'e_sd1',
    name: '兵士',
    classId: 'soldier',
    level: 4,
    x: 10,
    y: 5,
    affinity: 'ice',
    stats: st(22, 7, 0, 5, 6, 2, 6, 1, 10, 5),
    growth: st(70, 40, 0, 35, 35, 20, 30, 10, 0, 0),
    weapons: ['ironLance'],
    ai: 'guard',
  },
  {
    id: 'e_kn1',
    name: 'アーマーナイト',
    classId: 'knight',
    level: 5,
    x: 10,
    y: 3,
    affinity: 'ice',
    stats: st(26, 8, 0, 6, 3, 2, 11, 2, 14, 4),
    growth: st(80, 45, 0, 35, 20, 20, 45, 10, 0, 0),
    weapons: ['ironLance'],
    ai: 'guard',
  },

  // ---- 宝物庫を預かる二人と、街道と川際の見張り。
  // **魔物は出さない。** 屍兵の初出は第7章「壁が抱えていたもの」で、そこでミレイユが
  // 「顔を見てはいけません」と言うのがこの game の魔物の定義になる。第1章で雑魚として
  // 二体倒したあとでは、その場面は成立しない（specs/story/chapters/ch7.md）。
  {
    id: 'e_g1',
    name: '山賊',
    classId: 'brigand',
    level: 3,
    x: 4,
    y: 2,
    affinity: 'dark',
    stats: st(24, 8, 0, 4, 4, 1, 4, 0, 12, 5),
    growth: st(70, 45, 0, 30, 30, 15, 20, 5, 0, 0),
    weapons: ['ironAxe'],
    ai: 'guard',
  },
  {
    id: 'e_g2',
    name: '山賊',
    classId: 'brigand',
    level: 2,
    x: 15,
    y: 2,
    affinity: 'fire',
    stats: st(22, 7, 0, 4, 5, 1, 3, 0, 12, 5),
    growth: st(70, 45, 0, 30, 30, 15, 20, 5, 0, 0),
    weapons: ['ironAxe'],
    ai: 'guard',
  },
  {
    id: 'e_b3',
    name: '山賊',
    classId: 'brigand',
    level: 4,
    x: 17,
    y: 9,
    affinity: 'fire',
    stats: st(26, 9, 0, 6, 6, 1, 4, 0, 13, 5),
    growth: st(75, 45, 0, 35, 35, 15, 20, 5, 0, 0),
    weapons: ['handAxe'],
    ai: 'aggressive',
  },
  {
    id: 'e_a2',
    name: '狩人',
    classId: 'eArcher',
    level: 3,
    x: 2,
    y: 6,
    affinity: 'wind',
    stats: st(20, 6, 0, 7, 6, 2, 3, 1, 7, 5),
    growth: st(60, 40, 0, 50, 45, 20, 15, 10, 0, 0),
    weapons: ['ironBow'],
    ai: 'guard',
  },

  {
    id: 'e_boss',
    name: 'ハーゲン',
    classId: 'general',
    level: 8,
    x: 10,
    y: 1,
    affinity: 'thunder',
    stats: st(34, 11, 0, 8, 5, 4, 11, 4, 14, 4),
    growth: st(90, 55, 0, 40, 20, 25, 45, 15, 0, 0),
    weapons: ['steelLance', 'handAxe'],
    ai: 'boss',
    isBoss: true,
  },
];

/**
 * 第1章 クレハ。海沿いの街道が南から北へ抜け、川を渡り、クロガネの旗が上がった
 * 関門に突き当たる。渡河点は街道の橋と西の小橋の二つ。門は一マスしかない。
 * specs/story/chapters/ch1.md
 */
export const CH1: ChapterDef = {
  title: '第1章  「クレハへの道」',
  /*
   * 「クレハへの道」。**道が主役の盤面**なので、道が一本通っていないと章が成立しない。
   *
   * 出撃地点（南端）から中央の街道が北へ伸び、川を橋で渡り、村を左右に見ながら
   * 門へ突き当たる。他に行く道は無い —— それがこの章の一文そのもの。
   *
   * 城の中は三部屋。門から入れるのは玉座の間だけで、左右の宝物庫へは扉を開ける
   * しかない。鍵は自軍に 3 つしかなく、扉 2 と宝箱 2 で 4 回要る。全部は取れない。
   *
   * 東の縁は海。四辺を同じ厚さの岩で囲うと箱に見えるうえ、海沿いの道という
   * 設定とも噛み合わない。
   */
  map: [
    'wwwwwwwwwwwwwwwwwwww',
    'www.C.w...T..w.C.www',
    'www...D..F.F.D...www',
    'www...w......w...www',
    'wwwwwwwwwwGwwwwwwwww',
    'whh.,,....b..,,.hh~w',
    'wh.fb..V..b..V...h~w',
    'w~~~b~~~~~b~~~~~~~~w',
    'w.fSb...,,b.....f.~w',
    'w..F......b.....F.~w',
    'w.f...,,..b....f..~w',
    'w....f....b..f....~w',
    'w.........b.......~w',
    'wwwwwwwwwwbwwwwwwwww',
  ],
  objective: { kind: 'seize', x: 10, y: 1, label: '玉座の制圧' },
  deploy: 5,
  // 街道の上とその両脇。行軍の途中という格好で並べる
  starts: [
    { x: 10, y: 12 },
    { x: 9, y: 12 },
    { x: 11, y: 12 },
    { x: 10, y: 11 },
    { x: 9, y: 11 },
    { x: 11, y: 11 },
    { x: 8, y: 12 },
    { x: 12, y: 12 },
  ],
  enemies: ENEMIES,
  villages: [
    { x: 7, y: 6, weapon: 'handAxe', text: '親父の手斧だ。持って行ってくれ。あんたらが門を開けてくれるなら。' },
    { x: 13, y: 6, potion: 2, text: '内地へ逃げた学者の置き土産です。傷薬しか残っていませんが。' },
  ],
  chests: [
    { x: 4, y: 1, weapon: 'steelSword' },
    { x: 15, y: 1, gold: 2500 },
  ],
  shop: [
    { weapon: 'ironSword', price: 460 },
    { weapon: 'ironLance', price: 360 },
    { weapon: 'ironAxe', price: 270 },
    { weapon: 'handAxe', price: 300 },
    { weapon: 'javelin', price: 400 },
  ],
  scripts: CH1_SCRIPTS,
};
