...
import type { Unit, GameMap, Tile, TerrainType, Position, Weapon, ConsumableItem, ChapterData } from '../../src/core/types';
...
describe('Dance action', () => {
  ...
  it('non-dancer unit cannot dance', () => {
    ...
    const fighter = makeUnit('fighter1', { x: 0, y: 0 }, {
      classId: 'fighter',
    });
    const ally = makeUnit('ally1', { x: 1, y: 0 }, {
      hasActed: true,
...
describe('Steal action', () => {
  ...
  it('transfers first item from enemy to thief', () => {
    ...
    const thief = makeUnit('thief1', { x: 0, y: 0 }, {
      classId: 'thief',
      skills: ['steal'],
      stats: { hp: 18, str: 4, mag: 0, def: 3, res: 1, spd: 10, skl: 6, lck: 5, mov: 6, cha: 0, wil: 0 },
    });
    const enemy = makeUnit('enemy1', { x: 1, y: 0 }, {
      faction: 'enemy',
  ...
  it('requires SPD > enemy SPD', () => {
    ...
    const thief = makeUnit('thief1', { x: 0, y: 0 }, {
      classId: 'thief',
      skills: ['steal'],
      stats: { hp: 18, str: 4, mag: 0, def: 3, res: 1, spd: 5, skl: 6, lck: 5, mov: 6, cha: 0, wil: 0 },
    });
    const enemy = makeUnit('enemy1', { x: 1, y: 0 }, {
      faction: 'enemy',
      stats: { hp: 20, str: 8, mag: 0, def: 5, res: 0, spd: 7, skl: 5, lck: 3, mov: 3, cha: 0, wil: 0 },
