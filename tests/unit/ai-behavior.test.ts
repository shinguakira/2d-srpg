...
import type { Unit, GameMap, Tile, TerrainType, Position, AIBehavior, Weapon, WeaponType, ConsumableItem } from '../../src/core/types';
...
describe('AI behavior: survival', () => {
  ...
  it('uses healing item in retreat mode', () => {
    ...
    const vulnerary: ConsumableItem = {
      id: 'vulnerary', name: 'Vulnerary', type: 'consumable',
      uses: 3, maxUses: 3, effect: { kind: 'heal', amount: 10 },
    };
    const enemy = makeUnit('enemy1', { x: 2, y: 0 }, {
      aiBehavior: { type: 'survival' },
  ...
  it('cautious mode avoids risky attacks at 30-50% HP', () => {
    ...
    const player = makeUnit('player1', { x: 1, y: 0 }, {
      faction: 'player',
      equippedWeapon: makeWeapon('sword'),
      stats: { hp: 30, str: 20, mag: 0, def: 10, res: 5, spd: 15, skl: 15, lck: 10, mov: 5, cha: 0, wil: 0 },
      currentHp: 30,
    });
...
describe('AI behavior: thief', () => {
  it('moves toward nearest chest tile', () => {
    ...
    const action = decideAction(thief, map, allUnits, undefined, { openedChests: new Set(), visitedVillages: new Set() });
  ...
  it('interacts with chest when on it', () => {
    ...
    const thief = makeUnit('thief1', { x: 1, y: 0 }, {
      aiBehavior: { type: 'thief' },
    });
    const player = makeUnit('player1', { x: 0, y: 0 }, {
      faction: 'player',
      equippedWeapon: makeWeapon('sword'),
    });
  ...
  it('ignores already-opened chests', () => {
    ...
    const allUnits = new Map([['thief1', thief], ['player1', player]]);
    const action = decideAction(thief, map, allUnits, undefined, { openedChests, visitedVillages: new Set() });
...
describe('AI behavior: healer', () => {
  it('heals lowest-HP ally', () => {
    ...
    const healer = makeUnit('healer1', { x: 0, y: 0 }, {
      aiBehavior: { type: 'healer' },
      equippedWeapon: staff,
      inventory: [staff],
      stats: { hp: 18, str: 2, mag: 8, def: 2, res: 6, spd: 5, skl: 5, lck: 3, mov: 3, cha: 0, wil: 0 },
    });
    const woundedAlly = makeUnit('ally1', { x: 2, y: 0 }, {
      faction: 'enemy',
      currentHp: 5,
      stats: { hp: 20, str: 8, mag: 0, def: 5, res: 0, spd: 7, skl: 5, lck: 3, mov: 3, cha: 0, wil: 0 },
    });
    ...
    const allUnits = new Map([['healer1', healer], ['ally1', woundedAlly], ['player1', player]]);
  ...
  it('stays away from enemies when possible', () => {
    ...
    const staff = makeWeapon('staff', { might: 5, minRange: 1, maxRange: 2 });
    const healer = makeUnit('healer1', { x: 3, y: 0 }, {
      aiBehavior: { type: 'healer' },
      equippedWeapon: staff,
      inventory: [staff],
      stats: { hp: 18, str: 2, mag: 8, def: 2, res: 6, spd: 5, skl: 5, lck: 3, mov: 3, cha: 0, wil: 0 },
    });
    const woundedAlly = makeUnit('ally1', { x: 1, y: 0 }, {
      faction: 'enemy',
      currentHp: 5,
    });
    const player = makeUnit('player1', { x: 6, y: 0 }, {
      faction: 'player',
      equippedWeapon: makeWeapon('sword'),
    });
  ...
  it('never initiates combat', () => {
    ...
    const allUnits = new Map([['healer1', healer], ['player1', player]]);
    const action = decideAction(healer, map, allUnits);
...
describe('AI behavior: escort', () => {
  it('stays within 2 tiles of target unit', () => {
    ...
    const escort = makeUnit('escort1', { x: 0, y: 0 }, {
      aiBehavior: { type: 'escort', targetUnitId: 'vip1' },
      stats: { hp: 20, str: 8, mag: 0, def: 5, res: 0, spd: 7, skl: 5, lck: 3, mov: 5, cha: 0, wil: 0 },
    });
    const vip = makeUnit('vip1', { x: 4, y: 0 }, {
      faction: 'enemy',
    });
    const player = makeUnit('player1', { x: 8, y: 0 }, {
      faction: 'player',
    ...
    const allUnits = new Map([['escort1', escort], ['vip1', vip], ['player1', player]]);
  ...
  it('attacks threats near target', () => {
    ...
    const escort = makeUnit('escort1', { x: 0, y: 1 }, {
      aiBehavior: { type: 'escort', targetUnitId: 'vip1' },
      stats: { hp: 20, str: 8, mag: 0, def: 5, res: 0, spd: 7, skl: 5, lck: 3, mov: 5, cha: 0, wil: 0 },
    });
    const vip = makeUnit('vip1', { x: 2, y: 1 }, {
      faction: 'enemy',
    });
    const player = makeUnit('player1', { x: 3, y: 1 }, {
      faction: 'player',
  ...
  it('falls back to aggressive when target is dead', () => {
    ...
    const allUnits = new Map([['escort1', escort], ['player1', player]]);
    const action = decideAction(escort, map, allUnits);
...
describe('AI behavior: coordinated', () => {
  it('focus-fires same target when 3+ units', () => {
    ...
    const e1 = makeUnit('e1', { x: 0, y: 0 }, {
      aiBehavior: { type: 'coordinated', groupId: 'alpha' },
      stats: { hp: 20, str: 8, mag: 0, def: 5, res: 0, spd: 7, skl: 5, lck: 3, mov: 5, cha: 0, wil: 0 },
    });
    const e2 = makeUnit('e2', { x: 0, y: 1 }, {
      aiBehavior: { type: 'coordinated', groupId: 'alpha' },
      stats: { hp: 20, str: 8, mag: 0, def: 5, res: 0, spd: 7, skl: 5, lck: 3, mov: 5, cha: 0, wil: 0 },
    });
    const e3 = makeUnit('e3', { x: 0, y: 2 }, {
      aiBehavior: { type: 'coordinated', groupId: 'alpha' },
      stats: { hp: 20, str: 8, mag: 0, def: 5, res: 0, spd: 7, skl: 5, lck: 3, mov: 5, cha: 0, wil: 0 },
    });
    const p1 = makeUnit('p1', { x: 4, y: 0 }, {
      faction: 'player',
      equippedWeapon: makeWeapon('sword'),
    });
    const p2 = makeUnit('p2', { x: 4, y: 2 }, {
      faction: 'player',
      equippedWeapon: makeWeapon('sword'),
    });

    const allUnits = new Map([['e1', e1], ['e2', e2], ['e3', e3], ['p1', p1], ['p2', p2]]);
  ...
  it('falls back to aggressive with fewer than 3 group members', () => {
    ...
    const allUnits = new Map([['e1', e1], ['e2', e2], ['player1', player]]);
...
describe('AI behavior: ambush', () => {
  it('stays hidden when no player nearby', () => {
    ...
    const ambusher = makeUnit('ambush1', { x: 0, y: 0 }, {
    ...
    const allUnits = new Map([['ambush1', ambusher], ['player1', player]]);
