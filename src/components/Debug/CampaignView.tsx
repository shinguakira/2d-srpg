    ...
    <div className="debug-screen__reference" data-testid="debug-campaign-view">
      <div className="debug-screen__section">
        ...
        <p className="debug-screen__desc-text">
          Current screen: <span style={{ color: '#fbbf24', fontWeight: 700 }}>{currentScreen}</span>
        </p>
        <p className="debug-screen__desc-text">
          Current chapter:{' '}
          <span style={{ color: '#fbbf24', fontWeight: 700 }}>{currentChapterId ?? 'None'}</span>
        </p>
        <p className="debug-screen__desc-text">
          Completed chapters:{' '}
          {completedChapters.length > 0
            ? completedChapters.map((ch) => (
                <span
                  ...
                  style={{ background: '#22c55e', marginRight: 4 }}
                >
                  {ch}
                </span>
              ))
      ...
      <div className="debug-screen__section">
        ...
        <p className="debug-screen__desc-text">{roster.length > 0 ? roster.join(', ') : 'Empty'}</p>
          ...
          {deadUnitIds.length > 0 ? (
            deadUnitIds.map((id) => (
              <span
                key={id}
                className="debug-screen__badge"
                style={{ background: '#ef4444', marginRight: 4 }}
              >
                {id}
              </span>
            ))
      ...
      <div className="debug-screen__section">
        ...
        <p className="debug-screen__desc-text">
          Game mode: <span style={{ fontWeight: 700 }}>{gameMode}</span>
        </p>
        <p className="debug-screen__desc-text">
      ...
      <div className="debug-screen__section">
        ...
        <p className="debug-screen__desc-text">
          Gold: <span style={{ color: '#fbbf24', fontWeight: 700 }}>{gold}</span>
        </p>
        <p className="debug-screen__desc-text">
          Forge materials: {forgeMaterials.length > 0 ? forgeMaterials.join(', ') : 'None'}
        </p>
        <p className="debug-screen__desc-text">
          Bonus EXP pool: <span style={{ color: '#22c55e', fontWeight: 700 }}>{bonusExp}</span>
        </p>
      </div>
      ...
      <div className="debug-screen__section">
          ...
          {endingsSeen.length > 0
            ? endingsSeen.map((e) => (
                <span
                  key={e}
                  className="debug-screen__badge"
                  style={{ background: '#a855f7', marginRight: 4 }}
                >
                  {e}
                </span>
              ))
        ...
        <p className="debug-screen__desc-text">
          NG+ unlocked:{' '}
          <span style={{ color: newGamePlusUnlocked ? '#22c55e' : '#ef4444', fontWeight: 700 }}>
            {newGamePlusUnlocked ? 'Yes' : 'No'}
          </span>
        </p>
      </div>
