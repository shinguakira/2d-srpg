...
test.describe('Unit Movement', () => {
  ...
  test('clicking a player unit shows movement range', async ({ page }) => {
    ...
    const moveTile = page.locator('[data-testid="move-range-10-10"]');
    await expect(moveTile).toBeVisible();

    // Check another tile in range
    const moveTile2 = page.locator('[data-testid="move-range-12-10"]');
  ...
  test('clicking a valid movement tile shows action menu', async ({ page }) => {
    ...
    await page.click('[data-testid="tile-10-9"]');
    await page.waitForTimeout(200);
  ...
  test('Wait action moves unit and marks it as acted', async ({ page }) => {
    ...
    const unitAtNewPos = page.locator('[data-testid="tile-10-9"] [data-testid="unit-shigeru"]');
    ...
    const unitAtOldPos = page.locator('[data-testid="tile-11-10"] [data-testid="unit-shigeru"]');
  ...
  test('Escape cancels selection', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="move-range-10-10"]')).toBeVisible();
    ...
    await expect(page.locator('[data-testid="move-range-10-10"]')).not.toBeVisible();
  });
  ...
  test('Cancel in action menu returns to move target', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="move-range-10-9"]')).toBeVisible();
  ...
  test('clicking occupied tile switches selection to that unit', async ({ page }) => {
    ...
    const moveTile = page.locator('[data-testid="move-range-13-7"]');
    await expect(moveTile).toBeVisible();
  });
});
