...
test.describe('Chapter 4 — Ancient Horrors', () => {
  test('map loads with correct dimensions', async ({ page }) => {
    ...
    await expect(grid).toHaveAttribute('data-grid-width', '16');
  ...
  test('player units are at correct positions', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="tile-6-9"] [data-testid="unit-eirik"]')).toBeVisible();
    await expect(page.locator('[data-testid="tile-8-9"] [data-testid="unit-seth"]')).toBeVisible();
    await expect(page.locator('[data-testid="tile-5-9"] [data-testid="unit-lute"]')).toBeVisible();
    await expect(page.locator('[data-testid="tile-9-9"] [data-testid="unit-natasha"]')).toBeVisible();
  });
  ...
  test('enemy units are present and at correct positions', async ({ page }) => {
    ...
    // 9 enemies total
    await expect(page.locator('[data-testid="tile-3-7"] [data-testid="unit-ch4_soldier_1"]')).toBeVisible();
    await expect(page.locator('[data-testid="tile-12-7"] [data-testid="unit-ch4_soldier_2"]')).toBeVisible();
    await expect(page.locator('[data-testid="tile-7-5"] [data-testid="unit-ch4_fighter_1"]')).toBeVisible();
    await expect(page.locator('[data-testid="tile-2-5"] [data-testid="unit-ch4_fighter_2"]')).toBeVisible();
    await expect(page.locator('[data-testid="tile-10-3"] [data-testid="unit-ch4_mage_1"]')).toBeVisible();
    await expect(page.locator('[data-testid="tile-4-3"] [data-testid="unit-ch4_mage_2"]')).toBeVisible();
    ...
    await expect(page.locator('[data-testid="tile-7-0"] [data-testid="unit-ch4_boss"]')).toBeVisible();
  });
  ...
  test('dungeon terrain — walls and corridors', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="tile-15-0"]')).toHaveAttribute('data-terrain', 'wall');
    await expect(page.locator('[data-testid="tile-0-5"]')).toHaveAttribute('data-terrain', 'wall');

    // Throne at (7,0)
    await expect(page.locator('[data-testid="tile-7-0"]')).toHaveAttribute('data-terrain', 'throne');
    ...
    await expect(page.locator('[data-testid="tile-7-5"]')).toHaveAttribute('data-terrain', 'plain');
  ...
  test('boss is on the throne tile', async ({ page }) => {
    ...
    const throneTile = page.locator('[data-testid="tile-7-0"]');
