...
- [ ] Add `hoverMovementRange` and `hoverAttackRange` (as `Set<string>`) to `GameState` in `gameStoreTypes.ts`
- [ ] In `hoverTile` when `playerAction === 'idle'` and a unit exists at hovered tile:
...
- [ ] Clear hover ranges when hovering empty tile or when a unit is selected
- [ ] Only recompute when the hovered unit changes (not on every tile hover) for performance
- [ ] In `RangeOverlay.tsx`, render hover ranges with reduced opacity (e.g. 0.15 vs 0.3) to distinguish from selection
- [ ] Blue for player movement, red for player attack range and all enemy ranges
- [ ] Add `data-testid="hover-range-overlay"` to the hover range elements
- [ ] Add E2E test: hover player unit → blue + red range appears; hover away → range disappears

## Spec Update

- [ ] Update `specs/ui/hud.md` — add "Hover Range Preview" section documenting idle-hover range display behavior, colors, and opacity distinction from selection ranges
