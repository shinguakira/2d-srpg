 ...
 * 従士＝ゼス、魔道士＝テオ、僧＝ミナ、斧＝ガロン、門を塞ぐ男＝ヴァルガ。
 */
...
export const OPENING: Script = {
  ...
  lines: [
    ...
    L('ゼス', 'p_seth', 'right', '殿下、久田の砦が落ちました。黒鉄の旗を掲げた山賊どもです。村人を川際に追い詰めています。'),
    L('シゲル', 'p_shigeru', 'left', 'なら取り返す。南へ抜ける道はあの門しかない。他に行く場所もない。'),
    L('ゼス', 'p_seth', 'right', '王宮を出てから一睡もしておられません。皆も同じです。もしお望みなら――'),
...
export const ENDING: Script = {
  ...
  title: '第1章  終了',
  lines: [
    L('ゼス', 'p_seth', 'right', '門は我らの手に。勝利です、殿下。'),
...
export const DEFEAT: Script = {
  id: 'defeat',
  lines: [L('ゼス', 'p_seth', 'right', '殿下——！ ……申し訳、ありません。'), N('炎の聖剣は、ここで失われた。')],
};

/** 死亡時の一言（FE 恒例） */
const DEATH_QUOTES: Record<string, string> = {
  p_shigeru: '父上……まだ、南へ……',
  p_seth: '殿下の……盾に、なれましたか……',
  ...
  'p_seth|p_shigeru': [
    {
      ...
      lines: [
        L('ゼス', 'p_seth', 'right', '殿下。厳原を出てから、何も召し上がっていません。'),
        L('シゲル', 'p_shigeru', 'left', '腹は減っていない。'),
        L('ゼス', 'p_seth', 'right', '伺ったのは、減っているかどうかではありません。召し上がったかどうかです。'),
    ...
    },
    {
      ...
      lines: [
        L('ゼス', 'p_seth', 'right', '殿下、剣に無理があります。手首ではなく、腰で振ってください。'),
        L('シゲル', 'p_shigeru', 'left', '……父上と、同じことを言うのだな。'),
        L('ゼス', 'p_seth', 'right', '王も剣をお使いでした。……差し出がましいことを。'),
    ...
    },
    {
      ...
      lines: [
        L('シゲル', 'p_shigeru', 'left', 'ゼス。人を数として動かすのが、日ごとに楽になっている。'),
        L('ゼス', 'p_seth', 'right', '……存じております。'),
        L('シゲル', 'p_shigeru', 'left', 'それが怖い。'),
        L('ゼス', 'p_seth', 'right', '怖いとお思いのうちは、まだ数ではありません。私が申し上げます。楽になりすぎたら、申し上げます。'),
      ],
    },
  ],
  'p_garon|p_rina': [
